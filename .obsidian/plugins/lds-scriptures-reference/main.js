...
__export(main_exports, {
  default: () => LdsLibraryPlugin
});
module.exports = __toCommonJS(main_exports);

// src/LdsLibraryPlugin.ts
...
// src/suggestion/VerseSuggester.ts
...
var LANGUAGE_MAPPING = {
  ...
  pap: "Papiamento",
  pol: "Polski",
  por: "Portugu\xEAs (Brasil)",
  ept: "Portugu\xEAs (Portugal)",
  kek: "Q'eqchi'",
  quh: "Quechua-Bolivia",
  quc: "Quich\xE9",
  tah: "Reo Tahiti",
...
function getInnerHTML2(node, options) {
  return hasChildren(node) ? node.children.map((node2) => getOuterHTML2(node2, options)).join("") : "";
}
...
function existsOne2(test, nodes) {
  return nodes.some((checked) => isTag2(checked) && (test(checked) || existsOne2(test, checked.children)));
}
...
function convertOptionFormats(options) {
  ...
  (_a5 = opts.adapter) !== null && _a5 !== void 0 ? _a5 : opts.adapter = esm_exports3;
...
function findFilterElements(root2, selector, options, queryForSelector, totalLimit) {
  ...
  if (limit === 0)
    return [];
  const elemsNoLimit = sub.length === 0 && !Array.isArray(root2) ? getChildren3(root2).filter(isTag2) : sub.length === 0 ? (Array.isArray(root2) ? root2 : [root2]).filter(isTag2) : queryForSelector || sub.some(isTraversal) ? findElements(root2, [sub], options, limit) : filterElements(root2, [sub], options);
...
function find3(root2, query, limit = Infinity) {
  const elems = prepareContext(root2, esm_exports3, query.shouldTestNextSiblings);
...
function filterArray(nodes, match, xmlMode, root2) {
  return typeof match === "string" ? filter4(match, nodes, { xmlMode, root: root2 }) : nodes.filter(getFilterFn(match));
}
...
// node_modules/cheerio/node_modules/entities/dist/esm/escape.js
var getCodePoint2 = (
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
...
var foreign_content_exports2 = {};
__export(foreign_content_exports2, {
  SVG_TAG_NAMES_ADJUSTMENT_MAP: () => SVG_TAG_NAMES_ADJUSTMENT_MAP2,
  adjustTokenMathMLAttrs: () => adjustTokenMathMLAttrs2,
  adjustTokenSVGAttrs: () => adjustTokenSVGAttrs2,
  adjustTokenSVGTagName: () => adjustTokenSVGTagName2,
  adjustTokenXMLAttrs: () => adjustTokenXMLAttrs2,
  causesExit: () => causesExit2,
  isIntegrationPoint: () => isIntegrationPoint2
});
var MIME_TYPES2 = {
  TEXT_HTML: "text/html",
  APPLICATION_XML: "application/xhtml+xml"
...
// src/utils/dom.ts
function findAuthor(root2 = document) {
  const getText3 = (sel) => {
    var _a5, _b;
    return ((_b = (_a5 = root2.querySelector(sel)) == null ? void 0 : _a5.textContent) != null ? _b : "").trim();
  };
  let name = getText3(".author-name");
  let role = getText3(".author-role");
  if (!name) name = getText3("h1 + p");
  if (!role) role = getText3("h1 + p + p");
  return {
    name: name.replace(/^By\s+/i, "").trim(),
    role
  };
}
function queryBetweenIds($3, startId, endId) {
  const out = [];
  let node = $3(`#${startId}`);
  const end2 = $3(`#${endId}`);
  if (!node.length || !end2.length) return [];
  while (node.length) {
    out.push(node);
    if (node.is(end2)) break;
    node = node.next();
  }
  return out;
}
...
function parseURL(fullPath) {
  ...
  const kind = match[1] === "scriptures" ? "scriptures" /* Scriptures */ : "general-conference" /* ConferenceTalk */;
  const resourcePath = match[2];
  const langBeforeCast = (_a5 = url.searchParams.get("lang")) != null ? _a5 : "eng";
  const language = isAvailableLanguage(langBeforeCast) ? langBeforeCast : "eng";
  const id = url.searchParams.get("id");
  if (id === "" || id === null)
    throw new Error(
      `URL ${url} does not have an id query param. A specific paragraph within the page must be referenced through an id.`
    );
  const paragraphs = id.split(",").map((e) => e.split("-")).map((ps) => {
    if (ps.length === 1)
      return { kind: 0 /* Single */, id: ps[0] };
    else if (ps.length === 2)
      return {
        kind: 1 /* Range */,
        start: ps[0],
...
// src/utils/scripture.ts
async function fetchScripture(_url) {
  const url = parseURL(_url);
  if (url.kind !== "scriptures" /* Scriptures */)
    throw new Error(
  ...
  const $3 = load(response.json.content.body);
  ...
  const verses = url.paragraphs.flatMap((p) => {
    if (p.kind === 0 /* Single */) {
      return [$3(`#${p.id}`).text().trim()];
    } else if (p.kind === 1 /* Range */) {
      return queryBetweenIds($3, p.start, p.end).map(
        (e) => e.text().trim()
      );
    } else {
      return [];
...
var VerseSuggestion = class _VerseSuggestion {
  constructor(book, chapter, verseString, lang) {
    this.book = book;
    this.chapter = chapter;
    this.verseString = verseString;
    this.lang = lang;
    this.verseIds = verseString.split(",").map(
      (range) => range.split("-").map((verse) => `p${verse}`).join("-")
    ).join(",");
  }
  // factory function
  static async create(book, chapter, verseString, lang) {
    const suggestion = new _VerseSuggestion(
      book,
      chapter,
      verseString,
      lang
  ...
  getUrl(volumeTitleShort, bookTitleShort) {
  ...
  getShortenedName(bookTitle) {
    for (const [name, info] of Object.entries(bookData)) {
      if (info.names.some(
        (name2) => name2.toLowerCase() === bookTitle.toLowerCase()
      )) {
  ...
  async loadVerse() {
    ...
    const verses = scriptureData.verses.map((_verse) => {
      var _a5;
      const [_, verseNumber, text3] = (_a5 = _verse.match(/^(\d+)\s*(.*)$/)) != null ? _a5 : [
        null,
        "0",
        ""
      ];
    ...
    this.text = verses.map(({ verse, text: text3 }) => `> ${verse} ${text3}`).join("\n");
    this.previewText = verses.map(({ verse, text: text3 }) => `${verse} ${text3}`).join("\n");
  }
  render(el) {
...
// src/suggestion/VerseSuggester.ts
var FULL_VERSE_REG = /^:(?:\[(\w{3})\]\s+)?([123]*[A-z ]{3,}) (\d{1,3})(?:\s+|:)(\d+(?:-\d+)?(?:,\d+(?:-\d+)?)*):$/i;
var VerseSuggester = class extends import_obsidian2.EditorSuggest {
  constructor(plugin) {
    super(plugin.app);
    this.plugin = plugin;
  }
  onTrigger(cursor, editor, _) {
    ...
    const match = (_a5 = currentContent.match(FULL_VERSE_REG)) == null ? void 0 : _a5[0];
  ...
  async getSuggestions(context) {
    ...
    const language = (_a5 = fullMatch[1]) != null ? _a5 : preferredLanguage;
...
var LdsLibrarySettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian3.Setting(containerEl).setName("Scripture language").setDesc(
    ...
    new import_obsidian3.Setting(containerEl).setName("GitHub Repository").setDesc("View the source code and report issues.").addButton(
      (btn) => btn.setButtonText("Open in GitHub").setCta().onClick(() => {
        window.open(
...
// src/suggestion/ConferenceSuggester.ts
var import_obsidian7 = require("obsidian");

// src/ui/SpinnerModal.ts
var import_obsidian4 = require("obsidian");
var SpinnerModal = class extends import_obsidian4.Modal {
  constructor(app) {
    super(app);
  }
  onOpen() {
...
// src/ui/TalkParagraphPicker.ts
var import_obsidian5 = require("obsidian");

// src/ui/spinner.ts
async function withSpinner(container, task) {
  container.empty();
...
// src/ui/TalkParagraphPicker.ts
var TalkParagraphPicker = class extends import_obsidian5.Modal {
  constructor(app, resourcePath, language, onPick) {
    super(app);
  ...
  async fetchTalkBody() {
    ...
    const response = await withSpinner(
      this.contentEl,
      () => (0, import_obsidian5.requestUrl)({ url, method: "GET" })
    );
  ...
  async populateParagraphElements() {
    ...
    tpl.innerHTML = body;
    (_a5 = tpl.content.querySelector("footer")) == null ? void 0 : _a5.remove();
  ...
  makeSelection() {
    ...
    const ids = Array.from(this.selected.entries()).map(([id, _]) => `#${id}`).join(",");
    const idElements = this.contentEl.querySelectorAll(ids);
    const content = Array.from(idElements).flatMap(
      (el) => el.textContent !== null ? [el.textContent] : []
    );
    const author = findAuthor(this.contentEl);
    return {
      start: ranges[0][0],
      range: ranges.map(([a, b]) => a === b ? a : `${a}-${b}`).join(","),
      content,
      author
    };
  }
...
// src/ui/TalkSuggestModal.ts
var import_obsidian6 = require("obsidian");
var TalkSuggestModal = class extends import_obsidian6.FuzzySuggestModal {
  constructor(app, talks, onChoose) {
    super(app);
    this.talks = talks;
  ...
  onChooseItem(value, _evt) {
    var _a5;
    (_a5 = this.onChoose) == null ? void 0 : _a5.call(this, value);
  }
};

// src/utils/general-conference.ts
function toCalloutString(talk) {
  ...
  const byline = ["[!ldslib-byline]", author.name, author.role, monthText].map((s) => `>> ${s}`).join("\n");
  return [header, body, byline, ""].join("\n");
}

// src/suggestion/SingleSuggestion.ts
var SingleSuggestion = class {
  constructor(data2, text3) {
    this.data = data2;
    this.text = text3;
  }
  render(el) {
...
// src/suggestion/ConferenceSuggester.ts
var CONF_REG = /^:(?:\[(\w{3})\]\s+)?([aA]pril|[oO]ctober|[aA]pr|[oO]ct)\s+(\d{4}):$/;
var ConferenceSuggester = class extends import_obsidian7.EditorSuggest {
  constructor(plugin) {
    super(plugin.app);
    this.plugin = plugin;
    this.app = plugin.app;
  ...
  onTrigger(cursor, editor, _file) {
    var _a5, _b;
    const currentContent = editor.getLine(cursor.line).substring(0, cursor.ch);
    const match = (_b = (_a5 = currentContent.match(CONF_REG)) == null ? void 0 : _a5[0]) != null ? _b : "";
    if (!match) return null;
    return {
      start: {
        line: cursor.line,
  ...
  async getSuggestions(context) {
    ...
    const language = (_a5 = match[1]) != null ? _a5 : this.plugin.settings.language;
    if (!isAvailableLanguage(language))
      throw new Error(`${language} is not a valid language option`);
    ...
    const suggestion = new SingleSuggestion(
      { year, month, language },
  ...
  async selectSuggestion(suggestion, _evt) {
    if (!this.context) return;
    const { editor, start, end: end2 } = this.context;
    ...
    try {
      ...
      const $3 = load(response.json.content.body);
      let talkListElements = $3(
        "nav > ul > li > ul > li[data-content-type='general-conference-talk'] > a"
      );
      if (talkListElements.length === 0)
        talkListElements = $3("nav > ul > li > ul > li > a");
      talks = talkListElements.map((_, el) => {
        var _a5;
        const title = $3(el).find("p.title").text();
        const author = $3(el).find("p.primaryMeta").text();
        const _href = (_a5 = $3(el).attr("href")) != null ? _a5 : "";
    ...
    new TalkSuggestModal(this.app, talks, ({ title, href }) => {
      new TalkParagraphPicker(
        ...
        ({ start: startId, range, content, author }) => {
          const url2 = `${BASE_URL}${href}?lang=${language}&id=${range}#${startId}`;
          ...
          editor.replaceRange(callout, start, end2);
        }
      ).open();
    }).open();
  }
...
// src/LdsLibraryPlugin.ts
var LdsLibraryPlugin = class extends import_obsidian8.Plugin {
  async onload() {
